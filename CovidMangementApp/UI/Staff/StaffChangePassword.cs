...
namespace CovidMangementApp.UI.Staff
{
    public partial class StaffChangePassword : Form
    {
        ...
        public StaffChangePassword(String username)
        {
            InitializeComponent();
        ...
        private void btnChangePassword_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    string code = "SELECT SA_PASSWORD FROM STAFF_ACCOUNT WHERE SA_USERNAME=N'" + username + "'";
                    ...
                    try
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            string r_password = reader["SA_PASSWORD"].ToString();

                            clsDatabase.CloseConnection();

                            if (r_password.Equals(oldpassword))
                            {
                                string query = "UPDATE STAFF_ACCOUNT SET SA_PASSWORD=N'" + newpassword + "' WHERE SA_USERNAME=N'" + username + "'";
                        ...
                        {
                            Notification notification = new Notification("Không tìm thấy câu trả lời cũ");
                            notification.ShowDialog();
                            clsDatabase.CloseConnection();
                            return;
                        }
